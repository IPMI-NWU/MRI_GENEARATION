...
def conv2d(x, output_dim, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02, padding='SAME', name='conv2d', is_print=True):
...
def conv2d_11(x, output_dim, k_h=3, k_w=3, d_h=1, d_w=1, stddev=0.02, padding='SAME', name='conv2d', is_print=True):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [k_h, k_w, x.get_shape()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.conv2d(x, w, strides=[1, d_h, d_w, 1], padding=padding)
...
def conv2d_111(x, output_dim, k_h=1, k_w=1, d_h=1, d_w=1, stddev=0.02, padding='SAME', name='conv2d', is_print=True):
    with tf.variable_scope(name):
        w = tf.get_variable('w', [k_h, k_w, x.get_shape()[-1], output_dim],
                            initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.conv2d(x, w, strides=[1, d_h, d_w, 1], padding=padding)
...
def deconv2d(x, k, k_h=3, k_w=3, d_h=2, d_w=2, stddev=0.02, padding_='SAME', output_size=None,
    ...
    with tf.variable_scope(name):
        ...
        if not output_size:
            h_output, w_output = input_shape[1] * 2, int(input_shape[2] * 2)
